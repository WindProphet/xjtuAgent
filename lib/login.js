...
let recursive = (xa, url, service, web, iter) => {
  console.error(`i: ${iter}  url: ${url}  t: ${new Date().toJSON()}`);
...
module.exports = {
  login: (xa) => {
    ...
    console.error(`login loading ${new Date().toJSON()}`);
    request
      .get(url)
      .redirects(0)
      .end((err, res) => {
  ...
  auth: (
    ...
    console.error(`${service} queue ${new Date().toJSON()}`);
    xa.queue("login", () => {
      console.error(`${service} loading ${new Date().toJSON()}`);
